...
class State(typing.NamedTuple):
    ...
    def evaluation_function(self, weight):
        ...
        if(self.upper):
            ...
            if(result is not None):
                if(result == "draw"):
                    return 0 + len(self.upper_tokens) - len(self.lower_tokens) + self.num_throws - self.opponent_num_throws + 0.2*(x*max_dist - total_capture_distance) + 0.5*(max_dist - closest_capture_distance) + 3*opponent_token_in_danger - 0.2*(y*max_dist - total_losing_distance) - 0.5*(max_dist - closest_losing_distance) - 3*token_in_danger + 10*num_player_invincible_token - 10*num_opponent_invincible_token
                if(result == "upper"):
                    return 100 + len(self.upper_tokens) - len(self.lower_tokens) + self.num_throws - self.opponent_num_throws + 0.2*(x*max_dist - total_capture_distance) + 0.5*(max_dist - closest_capture_distance) + 3*opponent_token_in_danger - 0.2*(y*max_dist - total_losing_distance) - 0.5*(max_dist - closest_losing_distance) - 3*token_in_danger + 10*num_player_invincible_token - 10*num_opponent_invincible_token
                if(result == "lower"):
                    return -100 + len(self.upper_tokens) - len(self.lower_tokens) + self.num_throws - self.opponent_num_throws + 0.2*(x*max_dist - total_capture_distance) + 0.5*(max_dist - closest_capture_distance) + 3*opponent_token_in_danger - 0.2*(y*max_dist - total_losing_distance) - 0.5*(max_dist - closest_losing_distance) - 3*token_in_danger + 10*num_player_invincible_token - 10*num_opponent_invincible_token
            ...
            if(closest_capture_distance == out_of_range and closest_losing_distance == out_of_range):
                ...
                return w[0]*(len(self.upper_tokens) - len(self.lower_tokens)) + w[1]*(self.num_throws - self.opponent_num_throws) + w[2]*(x*max_dist - total_capture_distance) + w[3]*(x*max_dist - closest_capture_distance) + w[4]*opponent_token_in_danger + w[8]*num_player_invincible_token - w[9]*num_opponent_invincible_token

            return w[0]*(len(self.upper_tokens) - len(self.lower_tokens)) + w[1]*(self.num_throws - self.opponent_num_throws) + w[2]*(x*max_dist - total_capture_distance) + w[3]*(x*max_dist - closest_capture_distance) + w[4]*opponent_token_in_danger - w[5]*(y*max_dist - total_losing_distance) - w[6]*(max_dist - closest_losing_distance) - w[7]*token_in_danger + w[8]*num_player_invincible_token - w[9]*num_opponent_invincible_token

    def to_matrix(self, weight):
        actions_dict = {}
        matrix = []
        num = 0
        for player_action in self.actions():
            ...
            for opponent_action in self.opponent_actions():
                ...
                row.append(new_state.evaluation_function(weight))
